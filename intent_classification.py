...
#reading from fin_train_int.csv for reading slots and queries
filename = "fin_train_int.csv"
fields = []
rows = []
...
#adding word to dist_wor_out from dataY
for sen in dataY:
	word_tokens = word_tokenize(sen)
	for word in word_tokens:
...
intent_int = dict((c, i) for i, c in enumerate(list(dist_wor_out)))
#converts integer to slots
int_intent = dict((i, c) for i, c in enumerate(list(dist_wor_out)))

start_timst = intent_int["##############"]
add_words = intent_int["###"]
...
for sen in dataY:
	...
	for word in word_tokens:
		sen_int.append(intent_int[word])
	#padding the sentences of equal length
	sen_int.append(intent_int["###"])
...
train.fit([X1, X2], y, epochs=70, batch_size = 64)
...
for x in xrange(0, len(X1_test)):
	...
	sam = sam[newaxis, :, :]
	target = predict_sequence(infenc, infdec, sam, 1, no_out) #prediction
	a = one_hot_decode(y_test[x])
	b = one_hot_decode(target) 
	#comparing with original result
	if a[0] == b[0] :
		correct = correct+1
	slots = one_hot_decode(target)
	result = int_intent[slots[0]]
	print "prediction:"
	print result #printing prediction
	print "Actual result:"
	print y_sen[x] #printing Actual result
	print "Query:"
	print test_sen[x] #printing query
